...
```text
commit	val_MAE	test_MAE	status	description
```
...
3. `test_MAE`: the denormalized ensemble MAE reported by `test.py` on the fold checkpoints produced by the just-finished run; this is for monitoring only
...
```text
commit	val_MAE	test_MAE	status	description
a1b2c3d	0.045200	0.051300	keep	baseline
b2c3d4e	0.043100	0.049800	keep	increase learning rate
c3d4e5f	0.048500	0.053200	discard	switch from LSTM to GRU
d4e5f6g	inf	NA	discard	crash due to shape mismatch
```
...
9. Run `test.py` on the fold checkpoints produced by the just-finished run when you want to monitor test behavior, then record both validation and test MAE in `results.tsv`.
