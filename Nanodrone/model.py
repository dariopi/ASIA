...
class PhysicsResidualCausal(nn.Module):
    ...
    def __init__(
        ...
        self.thrust_net = nn.Linear(8, 3, bias=True)
        nn.init.zeros_(self.thrust_net.weight)
        nn.init.zeros_(self.thrust_net.bias)

        # Motor mixing: [u1², u2², u3², u4²] → Δomega (3)
        self.motor_mix = nn.Linear(4, 3, bias=False)
        nn.init.zeros_(self.motor_mix.weight)
