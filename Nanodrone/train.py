...
def initialize_fold_log(log_path: Path, fold_name: str) -> None:
    ...
    with log_path.open("w", encoding="utf-8") as handle:
        ...
        handle.write("epoch,train_loss_norm_mae,train_mae_norm,val_mae_norm\n")
...
def compute_mae_loss(
    ...
    y_teacher = y_tgt if teacher_forcing_ratio > 0.0 and torch.rand(1).item() < teacher_forcing_ratio else None
    y_hat, _ = model(u, y0, y_teacher=y_teacher)
    return torch.mean(torch.abs(y_hat - y_tgt))
...
def train_one_fold(
    ...
    optimizer = torch.optim.AdamW(
        model.parameters(),
        lr=config_pars["lr"],
        weight_decay=config_pars.get("weight_decay", 1e-4),
    )
    scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
        optimizer,
        T_0=config_pars.get("lr_T0", 50),
        T_mult=config_pars.get("lr_T_mult", 2),
        eta_min=1e-5,
    )

    # Build flat batched tensors from all training sequences
    all_u = torch.cat([s.u for s in train_list_norm], dim=0).to(device)    # (N_total, T, 4)
    all_y = torch.cat([s.y for s in train_list_norm], dim=0).to(device)
    all_y0 = torch.cat([s.y0 for s in train_list_norm], dim=0).to(device)
    N_total = all_u.shape[0]
    batch_size = config_pars.get("batch_size", 128)
    ...
    tf_start = config_pars.get("teacher_forcing_start", 0.5)
    tf_end = config_pars.get("teacher_forcing_end", 0.0)
    max_epochs = config_pars["max_epochs"]

    initial_val = evaluate_one_sequence(model, raw_validation, validation_norm, normalizer, device)
    best_val_mae_norm = float(initial_val["mae_norm"])
    initial_train = aggregate_metrics_across_sequences(model, raw_train_list, train_list_norm, normalizer, device)
    initial_line = f"0,{0.0:.6f},{initial_train['mae_norm']:.6f},{initial_val['mae_norm']:.6f}"
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        tf_ratio = tf_start + (tf_end - tf_start) * epoch / max_epochs
        ...
        for start in range(0, N_total, batch_size):
            ...
            bu = all_u[batch_idx]
            by = all_y[batch_idx]
            by0 = all_y0[batch_idx]

            optimizer.zero_grad()
            # True scheduled sampling: Bernoulli per batch, not always-on when ratio>0
            use_tf = torch.rand(1).item() < tf_ratio
            y_teacher = by if use_tf else None
            y_hat, _ = model(bu, by0, y_teacher=y_teacher)
            loss = torch.mean(torch.abs(y_hat - by))
            loss.backward()

            grad_clip = config_pars.get("grad_clip_norm", 1.0)
        ...
        val_metrics = evaluate_one_sequence(model, raw_validation, validation_norm, normalizer, device)

        avg_loss = epoch_loss / max(n_batches, 1)
        line = f"{epoch},{avg_loss:.6f},{train_metrics['mae_norm']:.6f},{val_metrics['mae_norm']:.6f}"
    ...
    final_train = aggregate_metrics_across_sequences(model, raw_train_list, train_list_norm, normalizer, device)
    final_val = evaluate_one_sequence(model, raw_validation, validation_norm, normalizer, device)
