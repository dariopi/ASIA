...
3. `test_RMSE`: the denormalized ensemble RMSE reported by `test.py` on the fold checkpoints produced by the just-finished run; this is for monitoring only
...
```text
commit	val_RMSE	test_RMSE	status	description
a1b2c3d	0.997900	1.102300	keep	baseline
b2c3d4e	0.993200	1.094800	keep	increase learning rate
c3d4e5f	1.005000	1.110500	discard	switch from LSTM to GRU
...
9. Run `test.py` on the fold checkpoints produced by the just-finished run when you want to monitor test behavior, then record both validation and test RMSE in `results.tsv`.
