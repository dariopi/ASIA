...
def compute_training_loss(
    ...
    tbptt = config_pars.get("tbptt_chunk", None)
    noise = config_pars.get("input_noise_std", 0.0)
    use_tbptt = tbptt is not None and hasattr(model, "get_initial_hidden") and hasattr(model, "forward_from_hidden")
    ...
    for seq in normalized_sequences:
        ...
        y  = seq.y.to(device)
        y0 = seq.y0.to(device)
        if model.training and noise > 0.0:
            u = u + torch.randn_like(u) * noise
        if use_tbptt:
            h = model.get_initial_hidden(y0)
            for s in range(0, u.shape[1], tbptt):
                e = min(s + tbptt, u.shape[1])
                yh, hn = model.forward_from_hidden(u[:, s:e], h)
                diff = yh - y[:, s:e]
                sse  = torch.sum(diff**2)
                total_sse = sse if total_sse is None else total_sse + sse
                total_count += diff.numel()
                h = tuple(hh.detach() for hh in hn) if isinstance(hn, tuple) else hn.detach()
        else:
            yh, _ = model(u, y0)
            diff = yh - y
            sse  = torch.sum(diff**2)
...
def train_one_fold(
    ...
    if config_pars.get("optimizer", "adam").lower() == "adamw":
        optimizer = torch.optim.AdamW(model.parameters(), lr=config_pars["lr"],
                                      weight_decay=config_pars.get("weight_decay", 1e-4))
    else:
        optimizer = torch.optim.Adam(model.parameters(), lr=config_pars["lr"],
                                     weight_decay=config_pars.get("weight_decay", 0.0))
    if config_pars.get("lr_scheduler", "plateau").lower() == "cosine":
        _ce = [0]
        scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
            optimizer, T_0=config_pars.get("cosine_T0", 100),
            T_mult=config_pars.get("cosine_Tmult", 2), eta_min=config_pars.get("lr_min", 1e-5))
    else:
        _ce = None
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            optimizer, mode="min", factor=0.5,
            patience=config_pars.get("plateau_patience", 2), threshold=1e-4, min_lr=1e-5)
